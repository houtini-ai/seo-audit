...
export function suggestInternalLinks(db: Database.Database, receiverUrlKey: string, anchor: string): { anchor: string; donors: LinkSuggestion[] } {
  ...
  // Match the MIME type before any charset parameter (mirrors isHtmlContentType).
  const candidates = db.prepare(
    "SELECT url, url_key, ipr, inlink_count FROM pages WHERE status_code=200 AND indexable=1 AND (LOWER(content_type) LIKE 'text/html%' OR LOWER(content_type) LIKE 'application/xhtml+xml%') AND url_key != ? ORDER BY ipr DESC LIMIT 80",
