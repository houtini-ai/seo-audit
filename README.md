...
- **Around 58 checks.** Crawlability, indexability (with the *reason* a URL isn't indexable), duplication and canonicalisation, on-page, structured data, internationalisation, security, performance, sitemap reconciliation, backlinks, and the GSC × crawl questions that only make sense when the two are joined.
