...
## Tools (current)
`refresh_property` (sync everything; `segments` opt-in for device/country) · `sync_gsc` · `start_crawl` · `inspect_urls` · `track_ranks` · `check_sync_status` · `check_crawl_status` · `list_properties` · `run_audit` · `query_audit` · `fix_finding` (finding→fix moat) · `list_checks` · `seo_audit_help` (overview + example prompts) · `data_location` (get/set data dir) · `keyword_volume` · `related_terms` · `pull_backlinks` (DataForSEO; needs Backlinks subscription) · `search_intent` (Labs; intent classification) · `page_lighthouse` (On-Page; lab CWV) · `competitors_domain` (Labs) · `page_intersection` (Labs; content gap) · `normalize_url` · `get_dashboard` (App UI) · `get_dashboard_data` (app-only) · `export_report` (shareable HTML).
