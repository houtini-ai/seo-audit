...
## Live QA — accuracy bug-hunt across ehi / directdrivewheels / simracingcockpit (2026-06-24)
Goal: zero false positives on the three sites Richard audits. Every finding below was checked against ground truth (raw GSC rows + live HTTP), not the stored verdict.
- [x] **FP #4 — `internal-links-to-redirects` fired site-wide on www-canonical sites** (`0bb417e`). Root cause: crawl seeded at the non-www apex (which 301s to www) + `hostFormForProperty('sc-domain:…')` keys everything to apex, so every page records a phantom apex→www hop. EHI showed "1,471 internal links to a redirect" as the **#2 finding** when **0 of 129,157 internal links use the apex host** (100% point at the 200-OK www URL, verified live). Fix: flag a link only if its raw href (fragment-stripped) is itself a redirect *source* (`from` hop in a chain). EHI → ~95 genuine Shopify `customer_authentication` 302s; ddw/simracing show none. _Root host-detection fix still open (#26)._
- [x] **FP #5 — `keyword-cannibalisation` counted incidental long-tail as "competitors"** (`e020adb`). No per-URL impression floor: simracing's **#1 finding "best vr headset" (priority 789, "10 URLs")** was one page at 59,570 impr / pos 1.1 + nine pages with **1–7 impressions each**. Now a URL competes only if it holds ≥10% of the leader's impressions AND ≥10 impressions (SQLite `MAX() OVER PARTITION BY query`). Noise drops; genuine splits survive; EHI "ehi" 17→3.
- [x] **FP #6 — `anchor-text-incoherent` pooled nav/chrome anchors** (`836a645`). The EHI homepage's 2,940 inbound "Home" breadcrumb links read as "never mentions 'ehi'"; Shopify "Write a review"/"(0)" chrome flagged a product page on the SKU-fragment query "32x7". Now excludes self-links, letter-less anchors, and a nav/CTA stoplist before requiring ≥3 genuine editorial anchors. EHI 4→2, simracing 14→12 (FPs removed, real coherence findings kept).
- [x] **Verified CORRECT (held up under zero-trust):** `orphan-with-impressions` (the "orphans" really have only self-links inbound; `inlink_count` correctly excludes self-links), `broken-internal-links` (all sampled simracing URLs genuinely 404 live, incl. one with 425 inbound links to a dead page), `ctr-below-expected` (beamng page really ranks pos ~2–3 with ~0% CTR — SERP-feature; the "verify" note is already present), and the freshness wiring (`gscFreshness().effectiveMax` + date-bounded `win`/`winPrev` — no cliff).
- [ ] **Known issue (#26):** non-www seeding still inflates `redirect-chain` / the indexability "redirects" count on www-canonical sites (low priority). Fix = detect canonical host at crawl start, key to it; needs a re-crawl.
- [ ] **Known residual (#28):** branded queries surface as pseudo-cannibalisation (sitelinks) and pseudo-anchor-incoherence (generic pages ranking for the brand). Fix = brand-aware query exclusion in both checks.
- [ ] **Stale-not-bug:** EHI `homepage-missing-org-schema` no longer reproduces — the live homepage now carries Organization JSON-LD (added after the 2026-06-22 crawl); a re-crawl clears it. _Consider timestamping findings against crawl date._
- _All three fixes are committed + pushed; they apply to the live MCP after the next Desktop restart._
